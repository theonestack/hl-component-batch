...
describe 'compiled component batch' do
  ...
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/fargate/batch.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "FargateSpotComputeEnvironment" do
      let(:resource) { template["Resources"]["FargateSpotComputeEnvironment"] }
      ...
      it "to have property ComputeResources" do
          expect(resource["Properties"]["ComputeResources"]).to eq({"MaxvCpus"=>10000, "SecurityGroupIds"=>{"Ref"=>"BatchSecurityGroup"}, "Subnets"=>{"Ref"=>"SubnetIds"}, "Tags"=>{"Environment"=>{"Ref"=>"EnvironmentName"}}, "Type"=>"MANAGED"})
      end
      
      it "to have property State" do
          expect(resource["Properties"]["State"]).to eq("DISABLED")
      end
    ...
    context "FargateSpotJobQueue" do
      let(:resource) { template["Resources"]["FargateSpotJobQueue"] }
